...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Runs Kraken OCR in no-segmentation mode on row images.")
...
def run_ocr(kraken_exe: Path, model_path: Path, src: Path, dst: Path, batch_size: int) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env["PYTHONUTF8"] = "1"
    command = [
        str(kraken_exe),
...
def main() -> None:
    ...
    manifest_path = output_dir / "kraken_results.csv"
